...
- **Revision (87d11ec):** Added `Implementation status` to clarify implementation-vs-deployment scope: repo confirms config guards (`Adm01InternalHttpConfig`), standalone entrypoint (`python -m app.internal_admin`), and enabled-mode allowlist; production network/transport controls remain environment responsibility (private network + trusted reverse proxy and/or mTLS; no public internet default).
