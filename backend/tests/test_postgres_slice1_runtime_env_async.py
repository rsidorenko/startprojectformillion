"""Opt-in async env runtime on PostgreSQL: DATABASE_URL + env builder + one /start iteration."""
...
_TG_USER = 8_888_888_800_930
_UPDATE_ID = 9_130
_CORR_ID = "b4e5f67890123456789abcdef0a1b2c3"
...
def pg_url() -> str:
    url = _database_url()
    if url is None:
        pytest.skip("DATABASE_URL not set; skipping PostgreSQL slice-1 env async runtime integration test")
    return url
...
def test_postgres_slice1_env_async_runtime_start_iteration_writes_rows(
    pg_url: str,
    ...
    async def main() -> None:
        ...
        try:
            async with httpx.AsyncClient(transport=transport) as ac:
                ...
                try:
                    summary = await app.run_iterations(1, correlation_id=_CORR_ID)
